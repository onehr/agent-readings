...
* **ClawBench: Can AI Agents Complete Everyday Online Tasks?** ([Zhang et al., 2026](https://arxiv.org/abs/2604.08523)): Moves browser-agent evaluation to 144 live production websites and write-heavy workflows while blocking each task's terminal request. The important systems idea is the combination of task-scoped effect interception, fresh isolated runs, and five synchronized evidence layers; the important trade-off is ecological validity versus exact rerun determinism. Read its scores as model-plus-harness results, not model-only capability.
